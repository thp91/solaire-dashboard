  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-[28px] font-semibold tracking-tight text-[#1d1d1f]">Modules</h1>
        <div className="flex gap-2 text-sm">
          <span className="bg-[#e5e5ea] text-[#FF9500] px-3 py-1 rounded-full font-medium">
            {unassigned.length} non assigné{unassigned.length > 1 ? 's' : ''}
          </span>
          <span className="bg-[#e5e5ea] text-[#34C759] px-3 py-1 rounded-full font-medium">
      ...
      {loading ? (
        ...
        <div className="bg-[#ffffff] rounded-2xl shadow divide-y divide-[#e5e5ea]">
          {devices.length === 0 && (
            <p className="p-6 text-[#6e6e73] text-sm">Aucun module enregistré.</p>
          )}
          {devices.map((d) => {
            const org = orgs.find((o) => o.id === d.organization_id);
            return (
              <div key={d.id} className="flex items-center gap-4 p-4">
                <span className={`w-2.5 h-2.5 rounded-full flex-shrink-0 ${d.organization_id ? 'bg-[#34C759]' : 'bg-[#FF9500]'}`} />

                <div className="flex-1 min-w-0">
                  <Link href={`/admin/devices/${d.id}`}
                    className="text-sm font-semibold text-[#1d1d1f] hover:text-[#0071e3] transition"
                  >
                    {d.name ?? d.id}
                  </Link>
                ...
                <div className="flex-shrink-0">
                  <select
                    ...
                    className="border border-[#e5e5ea] rounded-xl px-3 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-[#0071e3] disabled:opacity-50"
                  >
                ...
                {org && (
                  <Link href={`/admin/organizations/${org.id}`}
                    className="text-xs text-[#0071e3] hover:underline flex-shrink-0"
                  >
                    {org.name}
                  </Link>
                )}
